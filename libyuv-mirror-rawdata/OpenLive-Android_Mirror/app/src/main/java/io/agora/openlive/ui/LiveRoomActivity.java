...
import io.agora.preprocessing.VideoPreProcessing;
...
public class LiveRoomActivity extends BaseActivity implements AGEventHandler {
    ...
    protected void initUIandEvent() {
        ...
        ImageView buttonMirror = (ImageView) findViewById(R.id.btn_mirror);
        ...
        if (isBroadcaster(cRole)) {
            ...
            broadcasterUI(button1, button2, button3, buttonMirror);
        } else {
    ...
    private void broadcasterUI(final ImageView button1, ImageView button2, ImageView button3, ImageView buttonMirror) {
        button1.setTag(true);
        ...
        buttonMirror.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (flag) {
                    ...
                    worker().getRtcEngine().setParameters("{\"che.video.local.render\":false}");
                    mVideoPreProcessing.enablePreProcessing(true);
                    worker().getRtcEngine().setLocalVideoMirrorMode(Constants.VIDEO_MIRROR_MODE_DISABLED);//关闭本地镜像功能
                    worker().getRtcEngine().setParameters("{\"che.video.local.render\":true}");
                    android.util.Log.v("longxin","关闭本地镜像功能 : 打开裸数据镜像功能");
                    //add by longxin@agora.io
                    ...
                    //add by longxin@agora.io
                    worker().getRtcEngine().setParameters("{\"che.video.local.render\":false}");
                    worker().getRtcEngine().setLocalVideoMirrorMode(Constants.VIDEO_MIRROR_MODE_ENABLED);//打开本地镜像功能
                    worker().getRtcEngine().setParameters("{\"che.video.local.render\":true}");
                    mVideoPreProcessing.enablePreProcessing(false);
                    android.util.Log.v("longxin","打开本地镜像功能 : 裸数据接口关闭镜像功能");
                    //add by longxin@agora.io
                }
            }
        });
    ...
    private void doSwitchToBroadcaster(boolean broadcaster) {
        ...
        final ImageView buttonMirror = (ImageView) findViewById(R.id.btn_mirror);
        if (broadcaster) {
